...
MicroClaw 支持基于 OpenTelemetry (OTLP) 的可观测性能力，允许你追踪 Agent 的执行流程、监控 Token 消耗以及调试工具调用。我们提供了与 [Langfuse](https://langfuse.com/)（开源 LLM 工程平台）的一流集成支持。

### 功能特性
- **全链路追踪**：追踪 Agent 请求的完整生命周期（`agent_run`），包含 LLM 生成（`llm_generation`）和工具执行（`tool_execution`）。
- **指标监控**：监控输入/输出 Token 使用量、延迟和成本。
- **调试**：查看每一步的详细输入和输出，包括系统提示词（System Prompt）和工具返回结果。

### 配置指南

要启用可观测性，请在你的 `microclaw.config.yaml` 文件中添加以下配置段：
...
observability:
  ...
  # 如果是自托管部署: http://your-langfuse-instance:3000
  langfuse_host: "https://cloud.langfuse.com" 
  
  # 请从你的 Langfuse 项目设置中获取这些密钥
  langfuse_public_key: "pk-lf-..."
  langfuse_secret_key: "sk-lf-..."
```

配置完成后，重启 MicroClaw，你的 Agent 追踪数据将自动出现在 Langfuse 仪表盘中。

### 本地自托管 Langfuse 配置示例

如果你的 Langfuse 部署在本地（例如 `http://127.0.0.1:3000`），可以使用如下配置：
...
说明：
- `langfuse_host` 只需要填写 Langfuse Web/API 基础地址，不要加 `/api/public/otel/v1/traces`。
- MicroClaw 会基于 Langfuse 密钥自动构建 OTLP traces endpoint 和 Basic Auth 认证头。
- 如果 MicroClaw 运行在 Docker 容器中，不要使用 `127.0.0.1`，应改为容器可达地址（例如 `http://langfuse-web:3000`）。
